...
from tensorflow.keras.layers import LSTM, Dense, Dropout, SimpleRNN, Conv1D, MaxPooling1D, Flatten, TimeDistributed
...
st.title("📊 Hybrid LSTM-RNN-CNN Stock Price Forecasting and Prediction")
st.write("Advanced hybrid neural network combining LSTM, RNN, and CNN for time series prediction")
...
st.write("---")
st.header("🤖 Hybrid LSTM-RNN-CNN Model Training & Forecasting")
...
if st.button("🚀 Run Hybrid Model Forecast", type="primary"):
    with st.spinner("Training hybrid LSTM-RNN-CNN model... This may take a while."):
        try:
            # Scale data
            ...
            # Build hybrid model with CNN, RNN, and LSTM layers
            model = Sequential()
            ...
            # RNN layer for basic sequential processing
            model.add(SimpleRNN(units=lstm_units//2, return_sequences=True, activation='tanh'))
            model.add(Dropout(0.2))
            ...
            # Display results
            st.success(f"✅ Hybrid LSTM-RNN-CNN model trained successfully!")
            st.write("---")
            ...
            fig.update_layout(
                    title=f'{st.session_state.stock_name} - Hybrid Model Forecast Results',
                    height=1000,
                    hovermode='x unified',
                    legend=dict(
                        yanchor="top",
            ...
            fg = px.bar(
                ...
                x=test_dates, 
                y=residuals, 
                title='Residuals of Hybrid Model Forecast',
            ...
            st.error(f"❌ Error during hybrid model training: {str(e)}")

# Information about the Hybrid Model
with st.expander("ℹ️ About Hybrid LSTM-RNN-CNN Forecasting"):
    st.write("""
    **Hybrid LSTM-RNN-CNN Model** combines the strengths of three different neural network architectures for superior time series forecasting.
    
    **Model Architecture:**
    - **CNN Layers**: Capture local patterns and short-term features in the data
    - **RNN Layer**: Process sequential information with basic recurrent connections
    - **LSTM Layers**: Handle long-term dependencies and complex temporal relationships
    - **Dense Layers**: Final processing for prediction output
    
    **Key Advantages:**
    - **Multi-scale Pattern Recognition**: CNN captures local patterns, RNN handles sequences, LSTM manages long-term dependencies
    - **Robust Feature Extraction**: Multiple layers extract different types of features
    - **Improved Accuracy**: Combination of architectures often outperforms individual models
    - **Better Generalization**: Diverse layer types reduce overfitting
    
    **Parameters:**
    - **Lookback Window**: Number of previous time steps to use for prediction
    - **LSTM Units**: Number of neurons in LSTM layers (RNN uses half this amount)
