...
st.title("📊 Hybrid LSTM-GRU Stock Price Forecasting and Prediction")
st.write("Hybrid Long Short-Term Memory (LSTM) and Gated Recurrent Unit (GRU) neural network for time series prediction")
...
st.write("---")
st.header("🤖 Hybrid LSTM-GRU Model Training & Forecasting")
st.subheader("Hybrid Model Parameters")
...
if st.button("🚀 Run Hybrid LSTM-GRU Forecast", type="primary"):
    with st.spinner("Training Hybrid LSTM-GRU model... This may take a while."):
        try:
            # Scale data
            ...
            # Display results
            st.success(f"✅ Hybrid LSTM-GRU model trained successfully!")
            st.write("---")
            ...
            fig.update_layout(
                    title=f'{st.session_state.stock_name} - Hybrid LSTM-GRU Forecast Results',
                    height=1000,
                    hovermode='x unified',
                    legend=dict(
                        yanchor="top",
            ...
            fg = px.bar(
                ...
                x=test_dates, 
                y=residuals, 
                title='Residuals of Hybrid LSTM-GRU Forecast',
            ...
            st.error(f"❌ Error during Hybrid LSTM-GRU training: {str(e)}")

# Information about Hybrid LSTM-GRU
with st.expander("ℹ️ About Hybrid LSTM-GRU Forecasting"):
    st.write("""
    **Hybrid LSTM-GRU Model** combines the strengths of both Long Short-Term Memory (LSTM) and Gated Recurrent Unit (GRU) networks for enhanced time series forecasting.
    
    **Model Architecture:**
    - **LSTM Layer**: Captures long-term dependencies and complex patterns
    - **GRU Layer**: Provides efficient processing with fewer parameters
    - **LSTM Layer**: Final feature extraction for prediction
    
    **Key Advantages:**
    - **Better Feature Learning**: LSTM captures long-term patterns while GRU handles short-term variations
    - **Computational Efficiency**: GRU layers are faster to train than LSTM
    - **Improved Generalization**: Combination reduces overfitting
    - **Enhanced Pattern Recognition**: Leverages strengths of both architectures
    
    **Parameters:**
    - **Lookback Window**: Number of previous time steps to use for prediction
    - **LSTM Units**: Number of neurons in each LSTM/GRU layer
