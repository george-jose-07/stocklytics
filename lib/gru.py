...
st.title("📊 GRU Stock Price Forecasting and Prediction")
st.write("Gated Recurrent Unit (GRU) neural network for time series prediction")
...
st.write("---")
st.header("🤖 GRU Model Training & Forecasting")
...
with col2:
    gru_units = st.slider("GRU Units", 25, 100, 50, 5)
...
if st.button("🚀 Run GRU Forecast", type="primary"):
    with st.spinner("Training GRU model... This may take a while."):
        try:
            # Scale data
            ...
            # Build model with GRU instead of LSTM
            model = Sequential()
            model.add(GRU(units=gru_units, return_sequences=True, input_shape=(X_train.shape[1],1)))
            ...
            gru_rmse = np.sqrt(mean_squared_error(test.values, predictions))
            ...
            # Display results
            st.success(f"✅ GRU model trained successfully!")
            st.write("---")
            st.header("📈 Forecasting Results")
            col1, col2  = st.columns(2)
            with col1:
                st.metric("RMSE", f"{gru_rmse:.2f}", help="Root Mean Square Error")
            ...
            fig.update_layout(
                    title=f'{st.session_state.stock_name} - GRU Forecast Results',
                    height=1000,
                    hovermode='x unified',
                    legend=dict(
                        yanchor="top",
            ...
            fg = px.bar(
                ...
                x=test_dates, 
                y=residuals, 
                title='Residuals of GRU Forecast',
            ...
            st.error(f"❌ Error during GRU training: {str(e)}")

# Information about GRU
with st.expander("ℹ️ About GRU Forecasting"):
    st.write("""
    **Gated Recurrent Unit (GRU)** is a type of recurrent neural network that's particularly effective for time series forecasting.
    ...
    **Parameters:**
    ...
    - **GRU Units**: Number of neurons in each GRU layer
